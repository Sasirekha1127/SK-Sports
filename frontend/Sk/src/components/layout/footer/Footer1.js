...
export default function Footer1() {
	...
	useEffect(() => {
		fetch(`${apiUrl}/footer-settings`).then(r => r.json()).then(setFooter).catch(console.error)
		fetch(`${apiUrl}/settings`).then(r => r.json()).then(setSettings).catch(console.error)
	}, [apiUrl])

	// Fallback values
	const description = footer?.description || "Welcome to SK Sports Academy! Discover the joy of Badminton, connect with fellow enthusiasts, and unlock your full potential with expert coaching."
	const address = footer?.address || "2/364, Kalivelampatti Pirivu, Coimbatore - Trichy Rd, opp. Kongu Kalyana Mandapam, Palladam, Tamil Nadu 641662"
	const monTiming = footer?.mon_timing || "5:00 am–10:00 pm"
	const tueSatTiming = footer?.tue_sat_timing || "5:00 am–10:00 pm"
	const sunTiming = footer?.sun_timing || "5:00 am–10:00 pm"
	const copyright = footer?.copyright || "©2026 SK Sports. All Rights Reserved."
	const phone = settings?.phone || "8883422888, 978886004"
	const footerLogo = settings?.footerLogo || "images/logo-footer.png"
	const facebook = settings?.facebook || "#"
	const instagram = settings?.instagram || "#"
	const youtube = settings?.youtube || "#"

	// Split Tue-Sat timing by |
	const tueSatParts = tueSatTiming.split('|').map(s => s.trim())
					...
					<div className="themeflat-container">
						<div className="row footer-top">
							...
							<div className="col-xxl-4 col-lg-4 col-xl-4 col-md-12 logo-footer">
								<div className="widget">
									<div className="textwidget">
										<Link to="/" onClick={handleLogoClick} style={{ display: 'inline-flex', flexDirection: 'column', alignItems: 'flex-start', textDecoration: 'none' }}>
											<img id="a1" src={footerLogo} alt="SK Sports" style={{ height: '88px', width: 'auto' }} />
											<span className="logo-tagline" style={{
												...
												Train <span style={{ color: 'rgba(255, 255, 255, 0.45)', margin: '0 3px' }}>|</span> Compete <span style={{ color: 'rgba(255, 255, 255, 0.45)', margin: '0 3px' }}>|</span> Excel
											</span>
										</Link>
										<p>{description}</p>
										<div className="social-icon-footer">
											<a href={facebook} target="_blank" rel="noreferrer"><i className="icon-facebook" /></a>
											<a href={instagram} target="_blank" rel="noreferrer"><i className="icon-instagram" /></a>
											<a href={youtube} target="_blank" rel="noreferrer"><i className="icon-youtube" /></a>
										</div>
									</div>
								</div>
							</div>

							{/* Column 2 - Quick Links + Academy Timings */}
							<div className="col-xxl-4 col-lg-4 col-xl-4 col-md-6 link-footer">
								...
								<div className="widget widget_menu-footer">
									<h5 className="widget-title">Academy Timings</h5>
									<ul className="menu-footer timings-footer" style={{ color: '#fff', fontSize: '15px' }}>
										<li style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: '10px', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
											<span style={{ color: 'var(--primary)', fontWeight: '500' }}>Mon&nbsp;&nbsp;</span>
											<span style={{ textAlign: 'right' }}>{monTiming}</span>
										</li>
										<li style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: '10px', paddingTop: '10px', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
											<span style={{ color: 'var(--primary)', fontWeight: '500' }}>Tue–Sat&nbsp;&nbsp;</span>
											<span style={{ textAlign: 'right' }}>
												{tueSatParts[0]}
												{tueSatParts[1] && <><br />{tueSatParts[1]}</>}
											</span>
										</li>
										<li style={{ display: 'flex', justifyContent: 'space-between', paddingTop: '10px' }}>
											<span style={{ color: 'var(--primary)', fontWeight: '500' }}>Sun&nbsp;&nbsp;</span>
											<span style={{ textAlign: 'right' }}>{sunTiming}</span>
										</li>
									</ul>
								</div>
							</div>
							...
							<div className="col-xxl-4 col-lg-4 col-xl-4 col-md-6 new-letter">
								<div className="widget widget_text">
									...
									<p><i className="icon-MapPin" />{address}</p>
								</div>
							</div>
						</div>
						...
						<div className="row footer-bottom">
							...
							<div className="col-md-6 col-sm-12">
								<ul className="link-footer-bottom">
									<li>Developed by <Link to="https://saitechnosolutions.com" target="_blank">Sai Techno Solutions</Link></li>
								</ul>
							</div>
						</div>
					</div>
